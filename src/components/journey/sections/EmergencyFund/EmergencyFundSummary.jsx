...
import { TrendingUp, Shield, CheckCircle, Edit2, DollarSign, Target, Building2, Info } from 'lucide-react'
    ...
    <>
      ...
      <div className="bg-accent-purple-50 border-l-4 border-accent-purple-500 p-3 md:p-4 mb-6">
        <p className="text-sm text-gray-800">
          <strong className="text-accent-purple-900">Quick Check:</strong> Make sure your emergency fund is earning
          interest! If it&apos;s sitting in a regular checking/savings account earning 0%, consider moving it to a
          money market fund like SPAXX to earn ~5% annually.
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-300 rounded-xl p-4 mb-6">
        ...
        {!isEditing ? (
          ...
          <div className="space-y-4 mt-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Where do you keep it?</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Amount Saved</label>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setIsEditing(false)} className="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="bg-gray-50 border border-gray-300 rounded-xl p-4 mb-6">
          ...
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <dd className="font-semibold text-gray-900">{journeyData.existingEmergencyFundInstitution}</dd>
            </div>
            {journeyData.existingEmergencyFundType && (
              <div className="flex justify-between">
                <dt className="text-gray-600">Account Type:</dt>
                <dd className="font-semibold text-gray-900">{accountTypeLabel(journeyData.existingEmergencyFundType)}</dd>
              </div>
            )}
...
const EditButton = ({ onClick, title }) => (
  <button onClick={onClick} className="p-1 hover:bg-gray-200 rounded transition-colors" title={title}>
...
const SaveCancel = ({ onSave, onCancel }) => (
  <div className="flex gap-2">
    ...
    <button onClick={onCancel} className="px-3 py-1 bg-gray-300 text-gray-700 text-sm rounded hover:bg-gray-400">
      Cancel
    </button>
  </div>
)
      ...
      <div className="space-y-1 mb-8">
        <SummaryRow icon={Building2} label="Financial Institution">
          {editing === 'institution' ? (
            ...
            <div className="flex items-center gap-2">
              <span className="text-gray-900 text-base md:text-lg font-semibold">{institution}</span>
            ...
            <div className="space-y-2">
              {ACCOUNT_TYPES.map((t) => (
                <button
                  ...
                  className={`block w-full text-left px-3 py-2 rounded border-2 font-semibold text-gray-900 ${
                    t.value === accountType
              ...
              <button onClick={() => setEditing(null)} className="text-sm text-gray-600 hover:text-gray-800">
                Cancel
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <span className="text-gray-900 text-base md:text-lg font-semibold">{accountTypeLabel(accountType)}</span>
        ...
        <SummaryRow icon={Target} label="Goal Amount">
          <div className="text-gray-900 text-base md:text-lg font-semibold">${goal.toLocaleString()}</div>
        ...
        <SummaryRow icon={DollarSign} label="Starting Amount">
          {editing === 'amount' ? (
            ...
            <>
              <div className="flex items-center gap-2">
                <span className="text-gray-900 text-base md:text-lg font-semibold">${current.toLocaleString()}</span>
                <EditButton onClick={() => setEditing('amount')} title="Edit amount" />
              </div>
              {current === 0 && (
                <div className="text-sm text-gray-600 mt-1 italic">Starting from scratch - that&apos;s okay!</div>
              )}
            </>
          )}
        </SummaryRow>
        ...
        <SummaryRow icon={TrendingUp} label="Progress">
          ...
          {current < goal ? (
            <div className="text-sm text-gray-600">${(goal - current).toLocaleString()} remaining</div>
          ) : (
