  ...
  const JourneyFlow = () => {
    ...
    const getCurrentSteps = () => {
      ...
      const steps =
        typeof section.getSteps === 'function'
          ? section.getSteps(journeyData)
          : section.steps || []
    
      return steps.filter(Boolean) // ✅ strips undefined steps
    }
    ...
    const nextStep = () => {
      ...
      if (currentStepInSection < steps.length - 1) {
        ...
        if (currentIndex < sectionConfigs.length - 1) {
          const nextSection = sectionConfigs[currentIndex + 1]
          const nextSectionId = nextSection.id

          const nextSteps =
            typeof nextSection.getSteps === 'function'
              ? nextSection.getSteps(journeyData)
              : nextSection.steps || []

          const safeStep = Math.min(
            journeyData.lastStepInSection?.[nextSectionId] ?? 0,
            Math.max(0, nextSteps.length - 1)
          )

          setCurrentSection(nextSectionId)
          setCurrentStepInSection(safeStep)
        }
    ...
    // Go to section
    const goToSection = (sectionId, stepIndex) => {
      setCurrentSection(sectionId)
      setCurrentStepInSection(
        stepIndex ?? journeyData.lastStepInSection?.[sectionId] ?? 0
      )
      scrollToTop()
    }
    const getStepIndexInSection = (sectionId, stepComponent) => {
      const section = sectionConfigs.find(s => s.id === sectionId)
      if (!section) return 0
      const steps = section.getSteps ? section.getSteps(journeyData) : section.steps || []
      return steps.findIndex(step => step === stepComponent)
    }
